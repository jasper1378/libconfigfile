...
libconfigfile::parser::parser()
    : m_file_contents{},
      m_cur_pos{ m_file_contents.create_file_pos() }
{
}

libconfigfile::parser::parser(const std::string& file_name)
    : m_file_contents{ file_name },
      m_cur_pos{ m_file_contents.create_file_pos() }
{
}
...
void libconfigfile::parser::parse_file()
{
    ...
    while (m_cur_pos.is_eof() == false)
    {
        ...
        else if (is_pos_located_on_occurence_of(m_cur_pos, m_k_comment_cpp)) //TODO c/c++ comments
        {
            m_cur_pos.goto_next_line();
        }
        ...
        else if (m_file_contents[m_cur_pos] == m_k_directive_leader)
        {
            ...
            if ((test_pos.get_line() == m_cur_pos.get_line()) && (test_pos.is_bof() == false))
            {
                std::string what_arg{ "directive must be the only text on its line" };
                throw syntax_error::generate_formatted_error(m_file_contents, m_cur_pos, what_arg);
            }
...
void libconfigfile::parser::parse_directive()
{
    ...
    if (line_pos == std::string::npos)
    {
        ...
        throw syntax_error::generate_formatted_error(m_file_contents, m_cur_pos.get_line(), line_pos, what_arg);
    }
    ...
    else
    {
        ...
        throw syntax_error::generate_formatted_error(m_file_contents, m_cur_pos.get_line(), start_of_name, what_arg);
    }
}
...
bool libconfigfile::parser::is_pos_located_on_occurence_of(const file_pos& pos, const std::string& str)
{
    return ((pos.get_char()) == (m_file_contents.get_line(pos).find(str, pos.get_char())));
}

std::string libconfigfile::parser::get_substr_between_indices(const std::string& str, const std::string::size_type start, const std::string::size_type end)
{
